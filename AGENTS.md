...
- **How the summary is generated:** the script shells out to the local **`agy` agent CLI** in non-interactive print mode (`agy --print=… --output-format json`), one call per session. agy carries its own cached auth, so there's no per-job API key to manage under launchd. Model defaults to `gemini-3.6-flash` with reasoning effort `low` (override with `SESSION_LOG_MODEL` / `SESSION_LOG_EFFORT`); agy requires both a `--model` and an `--effort`. `--disable-slash-commands` stops any `/…` text in a transcript from being expanded. **Session transcripts are sent to agy's backing Gemini model** to be summarized.
...
- **Cost audit:** every agy call is logged as one JSONL line (`type:"call"` with session id, model, effort, input/output/total tokens, `est_cost_usd`, `logged`, and any `error`), plus a `type:"run"` summary line per run. Token counts come from agy's `usage` block (thinking tokens are folded into the output count, since Gemini bills them as output); cost is derived from configurable per-1M rates (`SESSION_LOG_PRICE_IN`/`SESSION_LOG_PRICE_OUT`, default `0.10`/`0.40`) and the rates are recorded in the run line so the log is self-describing. Written to `~/work/notes/logs/session-log-costs.jsonl` (override with `SESSION_LOG_COST_LOG`) — colocated with the notes but written directly, never through the git worktree, so it doesn't disturb the checkout. It's deliberately **not** gitignored: Obsidian's own sync/commit picks it up, so the audit trail is versioned without the script ever committing it.
...
- **Install:** `install_session_log_agent` in `install.sh` generates `~/Library/LaunchAgents/com.<user>.daily-session-log.plist` and `launchctl load -w`s it — **work machine only** (`hostname -s` == `$WORK_HOSTNAME`, like the skill-scan guard), and only when a `~/work/notes` git checkout exists. Logs to `~/.claude/logs/daily-session-log.log`. The script self-guards on the `agy` binary being on `PATH` and re-checks the repo, so a missing dependency just fails that run with a clear message.

## Session Transcript Archive (Google Drive)

Durable backup of **raw** Claude Code session transcripts (`~/.claude/projects/**/*.jsonl`) to Google Drive, so nothing is lost when Claude Code prunes old sessions locally (time-based `cleanupPeriodDays`, or aggressive disk-pressure cleanup). This is distinct from the Daily Session Log above, which sends *summaries* to notes; this archives the full JSONL. **All profiles** are captured (default + third-party), since the hook lives in `settings.base.json`.

Three defense-in-depth layers:

- **Layer 1 — long local retention.** `settings.base.json` sets `cleanupPeriodDays: 365`, so Claude Code keeps transcripts ~a year locally instead of the 30-day default. The archive becomes the durable store; local is just a long-lived working copy. (Only affects time-based cleanup, not disk-pressure pruning — Layer 2 covers that.)
- **Layer 2 — real-time capture (`SessionEnd` hook).** `.claude/hooks/archive-session.sh` (wired in `settings.base.json` under `SessionEnd`) copies the just-finished transcript into the mirror dir the instant a session ends — immune to any later cleanup or a skipped cron run. It reads `transcript_path` from the hook's stdin JSON, copies atomically (temp file + rename) to `<archive>/live/<encoded-project>/<session>.jsonl`, and **always exits 0** so it can never disrupt session end. Self-gates on the archive dir existing, so personal/Linux machines (no mirror set up) no-op. `SessionEnd` is best-effort (may not fire on a crash), which is why Layer 3 exists.
- **Layer 3 — weekday reconciliation sweep.** `scripts/archive-session-logs.sh` tars everything modified since the reference into a dated `<archive>/sweeps/<YYYY-MM-DD>.tar.gz` (temp + rename; same-day reruns overwrite that day's snapshot). Catches whatever the hook missed (crashes, other machines, backfill). Hardened so the sweep itself can't drop a session: **append-only** (never deletes from the archive, so a locally-pruned session's last snapshot persists) and **lookback overlap** — the scan reference is the *older* of the success watermark (`~/.claude/.archive-session-logs.watermark`) and a `CLAUDE_ARCHIVE_LOOKBACK_DAYS` floor (default 7), so a corrupted watermark or a skipped run never leaves a permanent gap, while a long outage triggers a full catch-up from the old watermark. Env knobs: `CLAUDE_ARCHIVE_DIR`, `CLAUDE_ARCHIVE_PROJECTS_DIR`, `CLAUDE_ARCHIVE_LOOKBACK_DAYS`, `CLAUDE_ARCHIVE_DRY_RUN=1` (lists what would be archived, writes nothing).
