...
- `keybindings.json` — Shared Claude Code keyboard-binding overrides. `install.sh` symlinks the single repo copy into every profile dir (same pattern as `status-line.sh`)
